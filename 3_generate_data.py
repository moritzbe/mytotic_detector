# Script to reduce false positives
...
# Aim: crop 64x64 flasely and correctly segmented masks. [n,x,y,channels+probability] = [n, 64,64,4]
...
positive_cells = np.empty([0,bounding_box_size,bounding_box_size,4])
negative_cells = np.empty([0,bounding_box_size,bounding_box_size,4])


for file_path in sorted(glob.glob(data_path + dataset + "0*_v2/"))[:-1]:
    ...
    #### Morphological Operations ####
        ...
        indices2 = np.unique(discard_array, return_counts=True)
        positive_centers = np.empty([0,0])
        for i in range(1, len(indices2[1])):
            x_center = np.round(np.mean(np.argwhere(discard_array==i)[:,0])).astype(int)
            y_center = np.round(np.mean(np.argwhere(discard_array==i)[:,1])).astype(int)
            code.interact(local=dict(globals(), **locals()))
            # positive_centers = np.concatenate((positive_centers, np.array([x_center, y_center]), axis=0))
